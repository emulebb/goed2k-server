...
- This repo is the active local ED2K server used by eMuleBB live E2E and
  parity scenarios.
...
- Keep the fork rebased on `chenjia404/goed2k-server` before behavior work,
  then apply eMuleBB integration deltas as small local commits.
- Resolve the eMule harness only through `EMULE_WORKSPACE_ROOT`.
- Before finishing Go changes, run:
  - `go test ./...`
  - `go build -o $env:EMULE_WORKSPACE_ROOT\workspaces\workspace\state\tools\goed2k-server\goed2k-server.exe .\cmd\goed2k-server`
